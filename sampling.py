...
############校正器
                ...
                # else:
                #   break
                print("Step: {}  PSNR:{:.5f} SSIM:{:.5f} MSE:{:.5f}".format(i, psnr0, ssim0, mse0))
                #  print("MAX PSNR:{:.4f} MSE:{:.4f}".format(max_psnr, min_mse))

                proj_ideal = Fan_ray_trafo(Fan_reco_space.element(img)).data
                #proj_ideal = gyh(proj_ideal)
                write_img(proj_ideal, 'ideal_sinogram.png')
                write_img(sinogram_input, 'sinogram_ldct.png')
                psnr1, ssim0, mse1 = indicate(tmp, proj_ideal)
                if psnr1 > max_psnr:
                    max_psnr = psnr1
                    # max_ssim = ssim0
                    min_mse = mse1
                # else:
                #   break
                # print("Step: {}  PSNR:{:.5f} MSE:{:.5f}".format(i, psnr1, mse1))
                # print("MAX PSNR:{:.5f} MSE:{:.5f}".format(max_psnr, min_mse))
