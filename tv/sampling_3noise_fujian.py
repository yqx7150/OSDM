...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model,data,mask,data_ob):
    ...
    with torch.no_grad():
      # Initial sample
      x_mean = sde.prior_sampling(shape).to(device)
      x1=x_mean
      x2=x_mean
      x3=x_mean
      ...
      for i in range(sde.N):
        ...
        #data_ob_first=data_ob.permute(2,0,1).unsqueeze(0)
        '''
        ...
        masked_data = masked_data_mean + torch.randn_like(x_mean) * std[:, None, None, None]
        a=1
        forward_fza_0,forward_fza_1,forward_fza_2=fza_mask_forward(masked_data)
        forward_fza_mean_0,forward_fza_mean_1,forward_fza_mean_2=fza_mask_forward(masked_data_mean)

        #############################x_mean#################################
        x_red=x_mean[0,0,:,:]+a*backward(forward_fza_0-forward(x_mean[0,0,:,:],mask),mask)
        x_green=x_mean[0,1,:,:]+a*backward(forward_fza_1-forward(x_mean[0,1,:,:],mask),mask)
        x_blue=x_mean[0,2,:,:]+a*backward(forward_fza_2-forward(x_mean[0,2,:,:],mask),mask)
        x_mean=torch.stack((x_red,x_green,x_blue),dim=0).unsqueeze(0)

        ###############################x####################################
        x_red=x_mean[0,0,:,:]+backward(forward_fza_mean_0-forward(x_mean[0,0,:,:],mask),mask)
        x_green=x_mean[0,1,:,:]+backward(forward_fza_mean_1-forward(x_mean[0,1,:,:],mask),mask)
        x_blue=x_mean[0,2,:,:]+backward(forward_fza_mean_2-forward(x_mean[0,2,:,:],mask),mask)
