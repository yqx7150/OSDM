...
####################################################################################
    ...
    def pc_sampler(img_model, check_num, predict, correct):
        with torch.no_grad():
            def gyh(inputs):
                ...
                return result_gyh
            max_psnr = 0
            img_List = np.load('../Test_CT/batch_img.npy')
    ...
    ############预测器
                    ...
                    z0 = x_new + 0.7 * (x_new - x)
                    x = x_new  # output
                    x0 = x
                    #x0 = gyh(x0)
                    x0 = tvdenoise(x0, 200, 10)  # TV去噪
                    x0=toNumpy(x0)
                    #x0 = gyh(x0)
                    x0 = padding_img(x0)
    ...
    ############校正器
                    ...
                    x_new = z0 - (w * norm_diff + 2 * hyper * sum_diff) / (2 * hyper + w)
                    z0 = x_new + 0.4 * (x_new - x)
                    x = x_new  # output
                    x0 = x
                    #tmp = np.copy(x)
                    #x0 = gyh(x0)
                    # x0 = tvdenoise(x0, 200, 20)  # TV去噪
                    # x0 = toNumpy(x0)
                    #x0 = gyh(x0)
                    tmp = np.copy(x0)
                    x0 = padding_img(x0)
                    ...
                    tmp = tvdenoise(tmp, 200, 200)  # TV去噪
                    tmp = toNumpy(tmp)
