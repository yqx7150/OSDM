...
class GetCT(Dataset):
    ...
    def __getitem__(self,index):
        ...
        # [768,768]
        # sinogram = sinogram / np.max(abs(sinogram))
        sinogram2 = np.zeros((768, 768,1))
        sinogram2[:, :,0] = sinogram[:, :]
        # -- now we get 768*768*coil
        ksize = [8, 8]
        ...
        # (579121, 64)
        A_temp = np.zeros((9048,64,64))
        ans_1 = np.array(A_temp, dtype=np.float32)
        for i in range(9048):  # diu 49
            cut = A[64 * i: 64 * (i + 1), :]
            ans_1[i, :, :] = cut
        # (9048, 64, 64)
        return ans_1
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetCT(root= "../train_img2")
  test_dataset = GetCT(root= "../train_img2")
